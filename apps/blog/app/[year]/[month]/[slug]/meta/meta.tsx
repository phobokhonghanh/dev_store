...
import Icons from '@origini/components/Icons'
import type { Post } from '@origini/interfaces'
import { distanceToNow } from '@origini/libs/date'
    ...
    <div>
      <div
        className={cn(
          ...
          'rounded-2xl bg-neutral-50 px-6 py-4 dark:bg-neutral-800',
      ...
      >
        ...
        <Link
          ...
          className="font-medium text-neutral-800 transition-colors hover:text-neutral-900 hover:underline hover:underline-offset-4 dark:text-neutral-200 dark:hover:text-neutral-100"
        >
          {post.category}
        </Link>
        <div className="flex flex-row flex-wrap gap-2">
          {post.tags.map((tag) => (
            <Link
              ...
              key={tag}
              title={`Tag: ${tag}`}
              className="rounded-full bg-neutral-200 px-3 py-1 text-xs font-medium text-neutral-700 transition-colors hover:bg-neutral-300 hover:text-neutral-900 dark:bg-neutral-700 dark:text-neutral-300 dark:hover:bg-neutral-600 dark:hover:text-neutral-100"
            >
              {tag}
            </Link>
          ))}
