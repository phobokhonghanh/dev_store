...
Lỗi xảy ra vì `WHERE` chạy *trước* `SELECT`. Tại thời điểm lọc, DB chưa hề biết `t` là cái gì.
...
### Ứng dụng thực tế:
*   **Ranking**: Tìm top 3 nhân viên lương cao nhất của *mỗi* phòng ban. (Dùng `ROW_NUMBER()` hoặc `DENSE_RANK()`).
...
*   *Tệ:* `WHERE YEAR(created_at) = 2024`. (Phải tính toán hàm YEAR cho 1 triệu dòng rồi mới so sánh -> Index vô dụng).
*   *Tốt:* `WHERE created_at >= '2024-01-01' AND created_at < '2025-01-01'`. (So sánh trực tiếp -> Index hoạt động).
